...
- `components/AddYourselfModal.tsx` — opened from a "+" trigger on `PathFinderPage`; lets a visitor enter a name and pick skills to simulate a gap/path against a role without being a seeded `Person`. State lives only in `PathFinderPage`'s component state (no `localStorage`), resets on reload, and is never sent anywhere that would create a `Person` node.
